...
이 브리핑의 목적은 단 하나입니다.
"이 글 하나만 읽고도 어제와 밤사이의 핵심 경제 흐름을 다른 사람에게 설명할 수 있게 한다."

전일 시장의 움직임뿐 아니라, 가격 반응이 아직 작아도 정책·산업·기술·국제정세·생활경제에
장기 영향을 줄 변화를 함께 포착해야 합니다. 제목만 훑으면 1분, 전체는 6~8분 안에 읽혀야 합니다.
...
■ 규칙 2-1 — 국내장과 미국장의 시간 순서를 뒤집지 않습니다
한국의 전 거래일 장 마감 원인은 그 장이 끝나기 전에 나온 정보만으로 설명하세요.
한국 장 마감 뒤 시작한 같은 날짜의 미국 장 결과를 한국 장의 원인으로 쓰지 마세요.
밤사이 미국 시장 결과는 다음 한국 거래일의 관전 요인으로만 연결하세요.
...

{
  ...
  "시장브리핑": [
    {"시장": "미국",
     ...
     "결과": "주요 지수·금리·원자재·특징주의 핵심 숫자로 무슨 일이 있었는지 2~3문장",
     "원인": "사건 이전의 배경, 이번에 달라진 점, 시장이 반응한 이유를 쉬운 말로 3~5문장",
     "전달경로": "사건 → 가격·금리 → 업종·지수 순서의 한 줄 인과 지도",
     "ETF연결": "한국 시장과 ETF에 어떤 순서로 이어질 수 있고 오늘 무엇을 확인해야 하는지 1~2문장",
  ...
  "월요일요약": {
    "지난주복기": ["지난주 국내·미국·대표 자산 흐름 중 핵심 1", "핵심 2", "핵심 3"],
    "주말변수": [
      {"제목": "금요일 장 마감 뒤 월요일 오전까지 새로 생긴 사건",
       "설명": "확인된 사실과 국내 증시·ETF로 전달될 경로",
       "출처": [{"이름": "매체명", "id": "입력 기사 id"}]}
    ],
    "국내개장영향": "지난주 흐름과 주말 변수를 종합해 월요일 국내장에서 확인할 업종·수급·ETF군"
  },
  ...
  "top5": [
    {"순위": 1,
     ...
     "영향": "증시·ETF 전달 경로 한 줄",
     "카톡요약": "사건과 원인 또는 의미가 함께 담긴 18~32자 완결 문장",
     "출처": [{"이름": "매체명", "id": "입력 기사 id"}]}
  ],
  ...
  "유튜브": [
    {"영상ID": "데이터에 있는 영상ID 그대로",
     ...
     "관련성": "높음|보통|낮음",
     "관련성근거": "ETF 아는형의 상시 콘텐츠 범위와 얼마나 가까운지 20자 이내"}
  ],
...
■ 분량 예산 — 이 브리핑 전체는 공백 포함 5,500~7,000자를 목표로 합니다.
핵심 이야기의 이해도를 지키는 것이 단순히 짧게 만드는 것보다 우선입니다.
...
경제 용어 해설은 별도 섹션으로 만들지 마세요. 실제 본문에 등장한 용어는 화면에서
자동 툴팁으로 설명합니다. 본문에 없는 개념을 교육용으로 억지로 추가하지 마세요.
...
def _stabilize_daily(fresh: dict, cached: dict, data: dict, cfg: dict) -> dict:
    ...
    # _postprocess는 입력 사전을 제자리에서 정리하므로, 그 전에 원본 카드를 복사한다.
    raw_markets = {x.get("시장"): dict(x)
                   for x in raw_fallback.get("시장브리핑", [])}
    ...
    result["오늘관전"] = _merge_unique(
        ...
        lambda x: str(x), 3)
    result["월요일요약"] = ((fresh.get("월요일요약") or cached.get("월요일요약"))
                             if data.get("브리핑역할") == "이번 주 준비" else None)
    ...
    if mode == "thursday":
        ...
        if mode == "weekly":
            extra = (
                ...
                "TOP5는 금요일 하루가 아니라 월~금 전체에서 시장을 움직인 사건을 골라\n"
                "시간 순서와 인과관계가 드러나게 쓰세요.\n"
                "입력의 `주간_대표흐름`은 대표 자산 1일·1주·1개월 비교표로 화면에 따로 표시됩니다.\n"
                "본문에서 표의 숫자를 전부 반복하지 말고 기간별 방향이 달라진 핵심만 해설하세요.\n"
                "시장브리핑과 특징주도 금요일 하루의 '오늘'이 아니라 한 주 흐름을 설명하세요.\n"
                "ETF 뉴스 6선은 목요일 전달문으로 분리했으므로 여기서는 빈 배열로 두세요.\n"
            )
        elif data.get("브리핑역할") == "이번 주 준비":
            label = "월요일 오전 브리핑"
            extra = (
                "\n이 브리핑의 역할은 회사에도 공유할 수 있는 '이번 주 준비'입니다.\n"
                "`월요일요약`에 ① 지난주 흐름 3줄 ② 금요일 장 마감 뒤 월요일 오전까지 새로 나온\n"
                "뉴스·정책·지정학 변수 ③ 월요일 국내 개장과 ETF로의 전달 경로를 작성하세요.\n"
                "금요일 종가는 짧게만 확인하고 이번 주 경제 일정과 확인할 수급·ETF군을 우선하세요.\n"
                ...
                try:
                    ...
                    if parsed:
                        ...
                        if mode == "daily":
                            gaps = _market_card_gaps(result.get("시장브리핑") or [])
                            if gaps:
                                log.warning("할당량 재시도 응답도 해설 부족: %s", "; ".join(gaps))
                                continue
                            result = _stabilize_daily(result, _load_daily_ai(data), data, cfg)
                            _save_daily_ai(data, result)
                        return result
...
def _build_daily_kakao(d: dict, data: dict, limit: int) -> dict:
    ...
    for i, item in enumerate(top[:5], 1):
        title = re.sub(r"\s+", " ", str(item.get("카톡요약") or item.get("제목") or "")).strip(" |·")
...
def _market_card_gaps(items: list[dict]) -> list[str]:
    ...
    for market_name in ("국내", "미국"):
        ...
        if len(str(card.get("결과") or "").strip()) < 80:
            missing.append("사건 설명 부족")
        if len(str(card.get("원인") or "").strip()) < 180:
            missing.append("배경·원인 설명 부족")
        if len(str(card.get("ETF연결") or "").strip()) < 70:
            missing.append("한국·ETF 연결 부족")
        if card.get("자동생성"):
...
def _postprocess(d: Any, cfg: dict, data: dict | None = None, mode: str = "daily") -> dict:
    ...
    for b in briefs:
        ...
        if market_name not in ("미국", "국내", "글로벌") or market_name in seen_markets or _has_hype(b):
            continue
        for key, limit_n in (("제목", 80), ("결과", 240), ("원인", 480),
                             ("전달경로", 180), ("ETF연결", 200)):
    ...
    monday = d.get("월요일요약") if isinstance(d.get("월요일요약"), dict) else None
    if data.get("브리핑역할") == "이번 주 준비" and monday:
        monday["지난주복기"] = [
            _strip_stock_flow_claim(x, data)[:100]
            for x in (monday.get("지난주복기") or [])
            if _strip_stock_flow_claim(x, data)
        ][:3]
        monday["주말변수"] = _drop_filler(monday.get("주말변수"), ("제목", "설명"))[:3]
        monday["국내개장영향"] = _strip_stock_flow_claim(
            monday.get("국내개장영향"), data)[:180]
        d["월요일요약"] = monday
    else:
        d["월요일요약"] = None
    d.pop("시장요약", None)
    ...
    for item in d.get("top5") or []:
        item["설명"] = _clip(item.get("설명"), 170)
        item["영향"] = _clip(item.get("영향"), 100)
        item["카톡요약"] = _clip(item.get("카톡요약") or item.get("제목"), 34)
